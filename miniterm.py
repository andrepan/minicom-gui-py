...
from serial_tool_layout import layout
from serial_tool_layout import sg
...
class Miniterm(object):
    ...
    def __init__(self, serial_instance, echo=False, eol='crlf', filters=(), w = None):
        ...
        try:
            while self.alive and self._reader_alive:
                ...
                if data and Filters in data:
                    if self.raw:
                        #print(Filters)
                        self.console.write_bytes("[{0}] {1}".format(dt.datetime.now().isoformat()[11:], data))                       
                        #self.console.write_bytes("Get:" + data)
                        self.window.Refresh()
                    else:
                        text = self.rx_decoder.decode(data)
                        for transformation in self.rx_transformations:
                            text = transformation.rx(text)
                        #self.console.write("Get" + text)
                        if self.ts:
                            text = f"[{dt.datetime.now().isoformat()[11:]}] " + text
                        self.console.write_bytes(text)
                        self.window.Refresh()
        except serial.SerialException:
            self.alive = False
            self.console.cancel()
...
def main(default_port=None, default_baudrate=9600, default_rts=None, default_dtr=None, w = None):
...
def get_ports_list():
    ports = []
    for n, (port, desc, hwid) in enumerate(sorted(comports()), 1):
        sys.stderr.write('{:7} {!r}\n'.format(port, desc[:5]))
        ports.append(port)
    return ports

def open_port(com_port, br = 115200, par = "N"):
    try:
        serial_instance = serial.serial_for_url(
                com_port,
                br,
        ...
        #sys.stderr.write('could not open port {!r}: {}\n'.format(com_port, e))
        sg.PopupError('could not open port {!r}: {}\n'.format(com_port, e))
...
def my_main():
    ...
    coms = get_ports_list()
    if len(coms) != 0:
        window['ports'].Update(values = coms, value = coms[0]) 
    while True: 
        event, values = window.read(timeout = 500) 
        if event == sg.WINDOW_CLOSED: 
            break
        elif event == "onoff":
            if window['onoff'].GetText() == "Open":
...
if __name__ == '__main__':
    #main(default_port="COM16", default_baudrate=115200)
    #t1 = threading.Thread(target=thread_print, name='rx')
    #t1.start()
    my_main()
    #print(get_ports_list())
